	...
	<div class="larry-personal">
		<div class="layui-tab">
			<blockquote class="layui-elem-quote news_search">
				...
				<?php if(in_array('banner/addbanner',$operable)): ?>
					<div class="layui-inline">
						<a href="<?php echo url('banner/addbanner'); ?>" class="layui-btn layui-btn-normal newsAdd_btn">添加幻灯片</a>
					</div>
				<?php endif; if(in_array('banner/sortbanner',$operable)): ?>
					<div class="layui-inline">
						<a data-url="<?php echo url('banner/sortbanner'); ?>" id="edit" class="layui-btn recommend" style="background-color:#5FB878">更新排序</a>
					</div>
				<?php endif; if(in_array('banner/delbanner',$operable)): ?>
					<div class="layui-inline">
						<a data-url="<?php echo url('banner/delbanner'); ?>" class="layui-btn layui-btn-danger batchDel">批量删除</a>
					</div>
				<?php endif; ?>
					...
					<tbody class="news_content">
						<?php if(is_array($banner['list']) || $banner['list'] instanceof \think\Collection || $banner['list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $banner['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
							<tr>
								...
								<td>
									<img src="<?php echo $vo['path']; ?>" style="width: 50px;height: 50px;" />
								</td>
								<td>
									<div class="layui-inline">
										<input name="sort" class="layui-input search_input" type="text" value="<?php echo $vo['sort']; ?>">
										<input name="id" type="hidden" class="bannerid" value="<?php echo $vo['id']; ?>" />
									</div>
								</td>
								<td>
									<?php if(in_array('banner/updatebanner',$operable)): ?>
										<a href="<?php echo url('banner/updatebanner',array('id'=>$vo['id'])); ?>" class="layui-btn layui-btn-mini "><i class="iconfont icon-edit"></i> 编辑</a>
									<?php endif; if(in_array('banner/delbanner',$operable)): ?>
										<a data-url="<?php echo url('banner/delbanner'); ?>" data-id="<?php echo $vo['id']; ?>" class="layui-btn layui-btn-danger layui-btn-mini delete"><i class="layui-icon"></i> 删除</a>
									<?php endif; ?>
